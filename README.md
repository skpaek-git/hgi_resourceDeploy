...
## 개요
`99. Deploy Resources.ps1`는 Excel 시트(`RG_*`, `VNET_*`, `LB_*`, `NSG_*`, `VM_*`)를 읽어 리소스를 배포합니다.
...
`8. Deploy DataDisk.ps1`는 `VM_DEV_Datadisk` 시트를 직접 읽어 데이터 디스크를 사전 생성합니다.
...
- 필수(행 공통): `VMName`, `RGname`, `Location`, `Zone`, `DESName`(+`DESRG`) 또는 `DiskEncryptionSetId`
...
- 실행 순서는 내부적으로 고정: `RG -> VNET -> STORAGE -> KV -> DES -> LB -> VM -> DATADISK -> NSG`
